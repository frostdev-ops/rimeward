...
try {
  ...
  assert.ok(calls.some(c => c.action === 'input' && c.events.some(e => e.type === 'key')));
  ...
  console.log('remote desktop UI smoke passed: view-only, acknowledged frames, control, input, expansion, phone layout, disconnect');
} finally {
  await browser?.close();
