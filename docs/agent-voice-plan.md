...
The original manual transport was validated on 2026-09-08. The Live session implementation and its separate validation gates are described first; earlier evidence is retained below.

## Implementation status

### Session-based Live voice (2026-09-21 development work)

The composer now exposes **Start voice session** / **End voice session**, and **Conversation → Off / Finish & Send / Live**. Live uses the separate `src/scripts/app/agent-live.ts` controller: one bidirectional WebRTC call remains open for the session, rather than replacing a dictation call with a read-aloud call after each message. The remote track plays through a hidden HTML audio element. Microphone capture requests echo cancellation, noise suppression and automatic gain control; there is no local RMS detector or silence-driven submission. **Speaker-safe** can pause outgoing microphone audio during voice-model speech without renegotiating the call, and is selected when the capture track does not report echo cancellation. This is a fallback, not proof of acoustic echo suppression.

Start requires a configured main Rime conversation and ChatGPT voice access. The server accepts only `mode: 'live'` and up to twelve existing-chat snippets totaling 12,000 UTF-8 bytes; it supplies the persona, fixed voice/model and `delegation: { type: 'client', ack_filler: true }` itself. It never accepts browser-supplied instructions, tool definitions or arbitrary session objects. Its receipt echoes the admitted mode, so a new client closes rather than silently treating an older server's manual session as Live. The credential-owning server must also run the Live implementation; updating only the desktop does not update a connected server.

Live captions are transient, bounded browser state and are erased when the session ends. They are never saved as Rime messages. A validated legacy V3 `delegation.created` item is framed visibly as lower-trust `<realtime_delegation>` input and enters the existing `submitText` / POST / steer path. The entire frame must fit the ordinary 8,000-character limit. The client records each delegation ID before asynchronous dispatch, refuses malformed/oversized requests and never retries an uncertain send. The server checks the current conversation, runtime and pending controls again at admission and before the message enters the turn. Voice cannot confirm, decline, answer an on-screen question or invoke an interrupt control. Existing tool permissions and approval rules still apply.

Completed Rime `says` / `reply` events return to the same realtime session, once, using the existing event identities; history replay and duplicate `done` payloads do not repeat them. A queued steer acquires a result binding only when the ordinary stream reports that it was absorbed. Draft text, mentions and attachments are not borrowed or cleared by voice delegation. Read responses and per-message manual read-aloud are disabled during Live so they cannot create competing calls. The provider supplies acknowledgement filler; there is no parallel custom progress narrator.

The optional state context is derived only from the already-held status snapshot, Rime busy/pending state and the ward's unread count. Each JSON digest is at most 500 UTF-8 bytes after escaping, carries observation time/staleness, and distinguishes unknown service status from healthy. A changed digest is offered at most once every thirty seconds; no endpoint or extra status polling was added. Backend context uses the pinned legacy V3 omitted-channel Thinking path. Its silent/retained behavior still requires live endpoint verification; the newer public GPT-Live event names and token limits are not substituted for this protocol.

Live remains active across dashboard page hiding and compact/expanded views. A global banner keeps Mute and End reachable, including from an open modal. Full navigation, ward deletion, a changed conversation/provider/runtime or failed session health ends it. End silences capture/playback immediately, then reports whether provider closure was confirmed. Late create responses are harvested for cleanup; uncertain calls retain their durable lease protection. Ending voice does not stop already-submitted Rime work.

**Validation boundary:** the pure digest/router/dedup checks and source checks do not establish provider behavior. The required six existing probe groups (nine synthetic calls), a persistent-session delegation/result check, quiet-context retention, `initial_items` acceptance, and signed-Mac open-speaker echo/interruption checks remain separate gates. No qualifying real-provider or signed-Mac acoustic acceptance result is recorded for this development revision. The earlier manual results below must not be presented as Live-session validation. Record those results against the exact candidate before accepting it as a Live voice release.

### Existing manual controls and earlier validation
...
The manual-flow gate is satisfied by the earlier user report. The retired Hands-free mode guessed at turn boundaries from microphone loudness followed by 2.5 seconds of silence and 1.2 seconds of transcript quiet, which was an explicit heuristic and never a provider finalization guarantee. That mode and the RMS-based barge-in sampler are removed. Finish & Send and manual Stop still drain for at least two seconds and 1.2 seconds of transcript quiet, with a six-second ceiling. An unsettled transcript stays editable. Earlier manual transport testing showed that `session.close` is not a transcript flush: closing about 800 ms after speech cut off punctuation.

In manual modes, only input transcript fragments edit the draft; provider delegation text and voice output have no automatic submission path. Newly received `says` and `reply` messages carry stable event identifiers shared by the direct stream and live mirror, preventing duplicate speech across those deliveries; stored history and repeated `done` data do not enqueue speech. Manual speech is sent sequentially in chunks of at most 500 UTF-8 bytes. The Live controller shares this byte-aware splitter and lease helpers, not manual draft handling, playback pumping or call replacement.
