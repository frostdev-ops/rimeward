...
Implementation target: web **0.23.2**, desktop **0.5.2**, remote-desktop protocol **1**.
...
| Local Developer ID signing | 96 nested binaries and six bundles signed; app version 0.5.1 passed deep strict verification; not notarized |
| Web deployment | 0.23.1 at `d41ba0f`; online database/application/nginx rollback backup retained, migrations 021/022 applied, database integrity and public/origin HTTP health passed |
| Apple Silicon installation | Signed 0.5.1 installed after graceful shutdown and profile backup; 29 wards and recovery records restored, paired server sync and authenticated public relay read passed |
...
| Installed macOS permissions | Both permissions report Allowed after the user's changes; authenticated paired-host capabilities report available screen, input, clipboard, files and audio. Installed 0.5.1 restored the page but lost the modal. The saved modal flag survived; a new cross-site bootstrap regression reproduced the missing restore marker. The short-lived metadata cookie now uses Lax so it reaches the initial app document; installed revalidation is pending |
...
| Distributed release | Tags `desktop-v0.5.0` and `desktop-v0.5.1` remain unchanged. Installer runs `34175413579` and `34176085313` were canceled before publication for the browser race and installed permission-modal failure respectively. Neither has a published release. The corrected release will use a new version; signing, notarization and publication remain unconfirmed |
