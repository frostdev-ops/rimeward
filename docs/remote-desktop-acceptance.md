...
| Dashboard keyboard ownership | Page shortcuts and dashboard Undo share a guard for handled events, IME composition, focused input surfaces and the original event path. Built regression checks passed browser digits/brackets, input and nested-note events after focus changes, composition, handled events and neutral dashboard navigation. Existing browser/editor/terminal/conversation and remote-workspace checks passed |
...
| Distributed release | Tags `desktop-v0.5.0`, `desktop-v0.5.1` and `desktop-v0.5.2` remain unchanged. Installer runs `34175413579`, `34176085313` and `34177028750` were canceled before publication for the browser race, installed permission-modal failure and dashboard shortcut regression respectively; none has a published release. The keyboard correction targets 0.5.3. Signing, notarization and publication remain unconfirmed |
