...
export const GET: APIRoute = async ({ url, cookies, redirect }) => {
  ...
  try {
    ...
    if(pending.brokerId){deliverBroker(pending.brokerId,pending.userId,{...tokens,access_token:tokens.access_token,label:claims.email??'Google'});return redirect('/oauth/broker?done=1',303);}
    storeLink({
