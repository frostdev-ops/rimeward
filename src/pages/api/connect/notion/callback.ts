...
export const GET: APIRoute = async ({ url, cookies, redirect }) => {
  ...
  if(pending?.brokerId && pending.userId && url.searchParams.has('error')){rejectBroker(pending.brokerId,pending.userId);return redirect(brokerDone(pending.brokerId,'notion','denied'),303);}
  ...
  try {
    ...
    if(pending.brokerId){deliverBroker(pending.brokerId,pending.userId,{...tokens,label:tokens.workspace_name??'Notion',meta:{workspace_id:tokens.workspace_id,bot_id:tokens.bot_id,rotating:!!tokens.refresh_token}});return redirect(brokerDone(pending.brokerId,'notion'),303);}
    storeLink({
    ...
    if(pending.brokerId){rejectBroker(pending.brokerId,pending.userId);return redirect(brokerDone(pending.brokerId,'notion','failed'),303);}
