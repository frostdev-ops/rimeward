...
export const GET:APIRoute = ({ locals }) => {
  ...
  const profile = embeddingProfile({ ...c,provider:'local',model:'Qwen3-Embedding-8B' });
  return Response.json({ sharing:c.sharing,profile:profile.id,quantization:c.quantization,ready:c.sharing && status.models.some(m => m.quantization === c.quantization && m.installed),
...
export const POST:APIRoute = async ({ locals,request }) => {
  try {
    ...
    const config = parseEmbeddingConfig({ ...c,provider:'local',model:'Qwen3-Embedding-8B' }), profile = embeddingProfile(config);
