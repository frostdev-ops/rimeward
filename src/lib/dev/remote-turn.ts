...
export function remoteTurnHealth() {
  ...
  probe = configured ? new Promise(resolve => {
    const socket = tls.connect({ host: 'turn.frostdev.io', port: 5349, servername: 'turn.frostdev.io', rejectUnauthorized: true });
...
/** coturn REST credentials are short-lived and scoped in audit to this account/session. */
export function remoteTurn(user: number, session: string) {
