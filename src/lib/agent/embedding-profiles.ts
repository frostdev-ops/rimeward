...
export const DEVICE_ID_RE = /^[a-f0-9-]{36}$/i;
/** A runtime that can host the local model: this runtime, the paired server (from a desktop), or a paired desktop. */
export const isRuntimeId = (id: unknown): id is string => id === 'local' || id === 'server' || (typeof id === 'string' && DEVICE_ID_RE.test(id));
export interface EmbeddingConfig {
  /** `local` is Qwen3-Embedding-8B on runtimes the user controls; cloud providers are a separate vector profile. */
  provider: 'local' | 'openai' | 'openrouter';
  quantization: Quantization;
  model: string;
  /** Priority order for `local`: the first runtime that answers is used, and one model variant serves them all. */
  runtimes: string[];
  sharing: boolean;
}
export interface EmbeddingProfile { id: string; model: string; dimensions: number; qwen: boolean; quantization?: Quantization }
export const DEFAULT_EMBEDDING: EmbeddingConfig = { provider: 'local', quantization: 'Q4_K_M', model: 'Qwen3-Embedding-8B', runtimes: ['local'], sharing: false };
export function parseEmbeddingConfig(raw: unknown): EmbeddingConfig {
  ...
  // `device` was the single-desktop provider before priority lists; it reads as a one-entry list.
  const legacyDevice = r.provider === 'device';
  if (!['local', 'device', 'openai', 'openrouter'].includes(String(r.provider))) throw Error('Choose an embedding provider.');
  const provider = (legacyDevice ? 'local' : r.provider) as EmbeddingConfig['provider'];
  ...
  if (provider === 'local') {
    if (legacyDevice) {
      if (typeof r.device !== 'string' || !DEVICE_ID_RE.test(r.device)) throw Error('Select a paired desktop.');
      runtimes = [r.device];
    } else if (r.runtimes === undefined) runtimes = ['local'];
    else {
      if (!Array.isArray(r.runtimes) || r.runtimes.length > 8 || !r.runtimes.every(isRuntimeId)) throw Error('Runtimes must list this computer, the paired server, or paired desktops.');
      runtimes = [...new Set(r.runtimes)];
    }
  }
  return { provider, quantization, model, runtimes, sharing: r.sharing === true };
}
