...
export const TOOLS: Record<string, ToolDef> = {
  monitor: {
    ...
    parameters:obj({ action:{ type:'string',enum:['create','update','pause','resume','delete','status'] },id:str('Monitor id for an existing monitor'),name:str('Short description'),minIntervalSeconds:num('Minimum seconds between alert deliveries for this monitor, 1–3600; default 5. Distinct from source.intervalSeconds (polling).'),
      source:{ type:'object',properties:{ type:{ type:'string',enum:['terminal','screen','file','browser','agent','note','notebook','http','comms','event'] },target:str('Terminal, ward, note, notebook or child task id'),
        for:str('screen: a Screen lens ward; `for` is a plain-words watch judged on device, `regex`/`rect`/`visual` narrow it; `for` is already triaged, so `decision` is for a second, different question'),regex:str('Regular expression over the changed text'),rect:{ type:'array',items:{ type:'number' },description:'[x,y,w,h]: only changes inside this rectangle' },visual:{ type:'boolean',description:'Purely visual changes count' },path:str('Virtual workspace file or folder. File monitors currently support local folders on this run’s runtime; remote and SSH file watchers are unavailable.'),url:str('Read-only HTTP(S) probe'),selector:str('Optional browser CSS selector'),event:str('Leyline trigger type'),intervalSeconds:num('5–86400 seconds, default 30; connector minimums still apply'),headers:{ type:'array',items:{ type:'string' } },fields:{ type:'array',items:{ type:'string' },description:'Selected JSON field paths' },
        watch:{ type:'object',description:'Lens sources only (terminal and screen): the long form of the fields above — hold deliveries until the watch hits; `for` is a plain-words intent, prefiltered by embedding and decided by triage when available; the reported mode is honest (unavailable/weak).',
          properties:{ for:str('What to wait for, in plain words'),regex:str('Regular expression over the changed text'),filter:{ type:'object',additionalProperties:{ type:'string' },description:'Header/text substrings that must all appear; on a screen `window` is the title the window was switched to and `title` is the live one' },rect:{ type:'array',items:{ type:'number' },description:'[x,y,w,h]: only changes inside this rectangle' },visual:{ type:'boolean',description:'Purely visual changes count' },triage:{ type:'boolean',description:'Send a hit to triage before delivering' },threshold:num('Embedding similarity floor, 0–1') },additionalProperties:false } },required:['type'],additionalProperties:false },
      filter:{ type:'object',description:'Exact filter: {all:[filters]}, {any:[filters]}, {not:filter}, or {field,op,value}; op eq, contains, glob (* and ?), gt, gte, lt, lte, changed. Maximum depth 8 and 64 nodes. Source fields include path, sender, channel, eventType, status, exitCode, text, and json fields. A lens-backed source (terminal, screen) emits {eventType:"key"|"delta", text, v, epoch, delivery, source} instead: `text` is the rendered observation, starting with the lens banner and a `d=` header, and the session state rides it as a `session=` header line — filter on text.',additionalProperties:true },
