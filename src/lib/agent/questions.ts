...
export function drainUserAnswer(conv: ConvRow): { item: unknown; text: string; questionId: string; query?: string } | null {
  return getDb().transaction(() => {
    ...
    return { item, text, questionId: q.id, query:q.answer === null ? undefined : Array.isArray(q.answer) ? q.answer.join('\n') : q.answer };
  })();
}
