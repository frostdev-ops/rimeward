...
export function attachWordEditor({ doc, tools, changed: onChanged, title, identity }: WordOptions) {
  ...
  button(tables, 'Table', async () => { remember(); const value = await askText('Table size: rows × columns (maximum 30 × 20)', '3 × 3'); if (!value || disposed) return; const match = /^(\d+)\s*[x×,]\s*(\d+)$/.exec(value.trim()); if (!match || +match[1] < 1 || +match[1] > 30 || +match[2] < 1 || +match[2] > 20) { message('Enter a size such as 3 × 3, up to 30 rows and 20 columns.'); return; } insert(`<table style="width:100%"><tbody>${Array.from({ length: +match[1] }, () => `<tr>${'<td><p><br></p></td>'.repeat(+match[2])}</tr>`).join('')}</tbody></table><p><br></p>`); });
  ...
  button(tables, 'Image URL', async () => { remember(); const url = await askText('Image URL (https://)'); if (!url || disposed) return; let valid: URL; try { valid = new URL(url); if (!['https:', 'http:'].includes(valid.protocol)) throw new Error(); } catch { message('Enter an HTTP or HTTPS image URL.'); return; } const alt = await askText('Describe this image for accessibility', 'Image'); if (alt === null || disposed) return; insert(`<img src="${escape(valid.href)}" alt="${escape(alt)}" style="max-width:100%;height:auto">`); });
  ...
  button(tables, 'Image file', () => { remember(); imageFile.click(); });
  imageFile.addEventListener('change', async () => { const file = imageFile.files?.[0]; imageFile.value = ''; if (!file) return; if (!/^image\/(png|jpeg|webp|gif)$/.test(file.type) || file.size > 3_000_000) { message('Choose a PNG, JPEG, WebP or GIF image under 3 MB.'); return; } const data = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = reject; reader.readAsDataURL(file); }).catch(() => ''); if (!data || disposed) return; const alt = await askText('Describe this image for accessibility', file.name); if (alt === null || disposed) return; insert(`<img src="${escape(data)}" alt="${escape(alt)}" style="max-width:100%;height:auto">`); }, { signal });
  ...
  button(review, 'Add comment', async () => { remember(); const range = currentRange(); if (!range || range.collapsed) { message('Select the passage to comment on.'); return; } const value = await askText('Comment'); if (!value || disposed || !doc.contains(range.commonAncestorContainer)) return; const mark = document.createElement('mark'); mark.dataset.comment = value.slice(0, 4000); mark.dataset.author = author; mark.append(range.cloneContents()); insert(mark.outerHTML, range); });
