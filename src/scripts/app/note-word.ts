...
export function attachWordEditor({ doc, tools, changed: onChanged, title, identity }: WordOptions) {
  ...
  const fontSelect = select(text, 'Font family', [['', 'Font'], ...['Arial', 'Calibri', 'Cambria', 'Georgia', 'Helvetica', 'Times New Roman', 'Courier New', 'Verdana', 'system-ui'].map(x => [x, x] as [string, string])], value => { if (value) native('fontName', value); });
  // The picker shows the font at the caret — the page's own (Calibri, the export default) until a run was styled.
  document.addEventListener('selectionchange', () => { const anchor = getSelection()?.anchorNode; if (!anchor || !doc.contains(anchor)) return; const name = String(document.queryCommandValue('fontName')).split(',')[0]!.replace(/["']/g, '').trim().toLowerCase(); fontSelect.value = [...fontSelect.options].find(o => o.value.toLowerCase() === name)?.value ?? ''; }, { signal });
  ...
  function refresh() { const repaired = repairPages(); if (identity() !== historyIdentity) record(); for (const mark of doc.querySelectorAll<HTMLElement>('[data-comment]')) mark.title = `${mark.dataset.author ?? 'Comment'}: ${mark.dataset.comment ?? ''}`; for (const change of doc.querySelectorAll<HTMLElement>('[data-change]')) change.title = `${change.tagName === 'DEL' ? 'Deleted' : 'Inserted'} by ${change.dataset.author ?? 'Unknown'}`; for (const [i, page] of [...doc.querySelectorAll<HTMLElement>('[data-word-page]')].entries()) for (const number of page.querySelectorAll<HTMLElement>('[data-page-number]')) number.textContent = String(i + 1); const page = doc.querySelector<HTMLElement>('[data-word-page]'); const [size, direction] = (page?.dataset.wordPage ?? 'letter-portrait').split('-'); pageSize.value = size; orientation.value = direction; margins.value = String(parseFloat(page?.style.padding ?? '') || 20); if (repaired) changed(); }
  refresh();
