...
// shares nothing with it: no peer connection, no lease, no read-aloud, no automatic send. It appends
...
export function createClipDictation(hooks: Hooks) {
  ...
  return {
    ...
    cancel,
    dispose: () => { epoch++; releaseMicrophone(); chunks = []; phase = 'idle'; },
...
export function downmix(buffer: AudioBuffer): Float32Array {
  ...
  for (let i = 0; i < length; i++) {
    ...
    let sum = 0;
    for (const channel of channels) sum += channel[low]! * (1 - t) + channel[high]! * t;
