...
// Worker accounts need traversal through the shared ancestor, not permission to list it.
for(const [directory,mode] of [['/var/lib/rimeward',0o711],['/var/lib/rimeward/workers',0o755],['/var/lib/rimeward/worker-tokens',0o700],['/run/rimeward-workers',0o755]]){
  await fs.mkdir(directory,{recursive:true,mode});const stat=await fs.lstat(directory);
  if(!stat.isDirectory()||stat.isSymbolicLink()||stat.uid!==0||(stat.mode&0o022))throw Error('Worker parent directory must be protected and root-owned.');
